...
class SubjectGeniusModel(L.LightningModule):
    def __init__(
        self,
        flux_pipe_id: str,
        lora_path: str = None,
    ...
    def step(self, batch):
        ...
        with torch.no_grad():
            ...
            # Prepare conditions
            if condition_types[0] in ["sam", "dinov2"]:
                condition_latents = Condition(condition_types[0], conditions[0]).condition
                condition_ids = torch.zeros((len(conditions), 3), device=self.device)
            else:
